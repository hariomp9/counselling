...
useEffect(() => {
  const fetchData = async () => {
    try {
      const response = await fetch("http://localhost:4000/api/category/getCategory");
      if (!response.ok) {
...
useEffect(() => {
  const fetchStates = async () => {
    try {
      const response = await axios.get('http://localhost:4000/api/state/getAllStates');
...
const handleSelectState = (state) => {
  ...
  axios.get(`http://localhost:4000/api/state_category/getStatesByCategory/${state}`)
    .then(response => {
  ...
  return (
    <>
      ...
      <Neetdetails onFormDataChange={handleFormDataChange} />
        ...
        <AllIndiaCategory categoryValues={categories}  onSelectCategory={setSelectedCategory}/>

        <div className="h-[1px] bg-[#E3E3E3] w-[100%] mt-[48px] mb-[33px]" />

        <DomicileStateCategory getStateCat={getStateCat} states={states}  onSelectState={handleSelectState} />
